...
async function main() {
  const aud: any[] = await prisma.$queryRaw`
    ...
    FROM auditoria WHERE tabela='cliente_prontuario' AND registro_id=1989 ORDER BY criado_em ASC
  `
  for (const a of aud) {
    ...
    console.log('ANTES (', antes.length, 'chars):', antes.slice(0,100))
    console.log('DEPOIS(', depois.length, 'chars):', depois.slice(0,100))
    console.log('DEPOIS fim:', depois.slice(-100))
    console.log()
  }
