  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Campanhas de Marketing</h1>
          <p className="text-slate-400 text-sm mt-1">Banners e comunicados exibidos no portal do cliente</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Campanhas Encerradas', val: campanhas.filter(c => !c.ativa).length, cor: 'text-slate-400' },
        ].map(k => (
      ...
      {mostrarForm && (
        <div className="card border border-blue-500/30">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Nova Campanha</h3>
      ...
      <div className="space-y-4">
        {campanhas.map(c => (
          <div key={c.id} className={`card border ${c.ativa ? 'border-slate-700' : 'border-slate-800 opacity-60'}`}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3 flex-1">
                <div className={`p-2 rounded-lg flex-shrink-0 ${c.ativa ? 'bg-blue-500/10' : 'bg-slate-700'}`}>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`badge text-xs ${c.ativa ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-500'}`}>
                      {c.ativa ? 'Ativa' : 'Encerrada'}
                    </span>
                    <span className="badge bg-slate-700 text-slate-400 text-xs">{c.tipo}</span>
                  </div>
