  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Gestão e ranking de comissões da equipe</p>
      </div>

      <div className="flex gap-2 border-b border-slate-700">
        {abas.map((a, i) => (
          <button key={a} onClick={() => setAba(i)}
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${aba === i ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
            {a}
          </button>
        ))}
      </div>
            ...
            <tbody>
              {comissoes.length === 0 ? (
                ...
                <tr key={c.id} className="table-row">
                  <td className="table-cell font-medium text-slate-100">{c.vendedorNome || '—'}</td>
                  <td className="table-cell text-slate-400">{c.clienteNome || '—'}</td>
                  <td className="table-cell"><span className="badge bg-slate-700 text-slate-300 text-xs">{c.descricao || '—'}</span></td>
                  <td className="table-cell text-slate-400">{formatDataVenda(c.dataVenda)}</td>
