  ...
  const Field = ({ label, field, prefix = '', step = 1 }: { label: string; field: string; prefix?: string; step?: number }) => (
    <div>
      ...
      <div className="relative">
        {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{prefix}</span>}
        <input
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">🚗 Deslocamento</h3>
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">🎓 Treinamento</h3>
            <div className="grid grid-cols-3 gap-4">
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">🏨 Hospedagem e Alimentação</h3>
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">💾 Migração e Desconto</h3>
            <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div className="card sticky top-6">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">💰 Resumo do Orçamento</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={item.label} className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-800 dark:text-slate-200">R$ {item.val.toFixed(2).replace('.', ',')}</span>
                </div>
              ))}
              <div className="border-t border-slate-200 dark:border-slate-700 pt-3 flex justify-between text-sm">
                <span className="text-slate-600 dark:text-slate-400">Subtotal</span>
                <span className="text-slate-800 dark:text-slate-200">R$ {subtotal.toFixed(2).replace('.', ',')}</span>
              </div>
              {form.desconto > 0 && (
